...
Before launching the notebook, you may need to set the separate code and data roots:
...
The first notebook cell changes its working directory to the analysis root so historical relative data paths continue to work, then prints `bstpp.__file__`. Stop if that path does not point into the preview checkout.
...
Then open `cbg-park-seasonal-refactor-preview.ipynb`. Run data preparation, one short single-park fit, and diagnostic cells before starting the full batch.