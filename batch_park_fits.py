...
# Default Hawkes trigger windows (see module docstring / get_park_fit_kwargs)
DEFAULT_FIT_KWARGS = {
    "window": 100.0,
...
def get_park_fit_kwargs(
    ...
    **overrides,
    ...
    """Merge default Hawkes settings (window, spatial_window, num_samples) for one park."""
...
def save_fit_outputs(
    ...
    *,
    ...
    """Save all figures; optionally display SVI loss, spatial map, and text summary."""
    label = PARK_LABELS.get(park_name, park_name)
...
def fit_one_park(
    ...
    _log(
        ...
        f"window={window} | spatial_window={spatial_window:g} m | "
...
def run_all_park_fits(
    ...
    Per fit, all diagnostic PNGs are saved; only SVI loss, spatial map, and the
    text parameter summary are shown inline (plus ``fit_summary.csv`` for the batch).
    ...
    Other per-park Hawkes settings (window, num_samples) use ``park_fit_overrides``.
    """
