...
**Phase 3 audit status (pre-3f audited tip** `e0d7e437`**; living record `phase3_record.tex`).** The prepared-data, event-indexed-state, polygon-support, and cutoff contracts below are enforced at tip. A-21 records the freeze tip `938e32b`, the first follow-up series (`df06e55`--`04799d9`), the continuation on `04799d9` (one-sided TruncatedNormal intersection, rejection of truncated non-Normal bases, rectangle computational-grid simulate dedup, production slope-doc correction), and the local correctness/API series on `e0d7e437` (`af7c934`--`c501283`: kernel capability gates, untouched-axis provenance, LGCP `set_window` rejection, panel/`min_sigma` guard, grid/kernel/MCMC helpers, runtime packaging metadata, stale-doc retargets; deferred membership consolidation and extreme float32 TLN notes). Acceptance evidence lives in `refactor-patches/phase3{a,c,d,e}/rebaseline_record.md`, `refactor-patches/phase3_tip_verification_2026-07-24.md`, `refactor-patches/confirmation_8580364.md`, and `refactor-patches/pre3f_audit_e0d7e43.md`. The living Phase 3 contract/decision record is the repository-root `phase3_record.tex` (Part I frozen + Part II append-only amendments **through A-58 / D-63**). Add a regression test that fails for any newly identified defect before changing production code.

**Phase 3f status — the manifest governs, and this file does not restate it.** `docs/phase3f_completion_manifest.yaml` is the **sole source of current Phase 3f execution truth**: scope, status, dependencies, open decisions, gate applicability, corrective-cycle counts, and completion (**D-53**). Its **evidence baseline is `c4e069f`** and it was **adopted at A-54**. `phase3_record.tex` remains the append-only **scientific and historical** authority — settled model semantics, change classifications, amendments. A status-changing commit updates the manifest **in that same commit**. The manifest is YAML and sits **outside** the document-prose gate populations (`phase3_record.tex`, `docs/*.md`, `AGENTS.md`), so a manifest-only edit runs the manifest validator and nothing else. The older planning files — `docs/wp_dependency_graph.md`, `docs/seam_sets.md`, `refactor-patches/phase3f/phase3f_work_package_outline.md`, the WP entry records — carry a supersession banner and are **historical evidence, never parallel task trackers**.

- **Execution shape.** Four vertical slices **S1–S4** under `docs/phase3f_execution_protocol.md`: S1 configuration spine (WP2, remaining WP1, WP3), S2 runtime ownership (WP4–WP6), S3 artifacts/interfaces (WP7–WP9), S4 cutover (WP10). **Analysis may resume at the S3 boundary** (**D-62**) — that milestone is *not* the Phase 3f exit and **moves no phase boundary**: 3f still closes at S4, and 3g still owns the Stage 3 SBC run and the `cbg-park-seasonal` confirmation.
- **OP-28 is CLOSED** (**D-54**) by the explicit `domain_area` decision, which **required no code change** — it settles intent, and both facts were already the shipped behaviour. The accept set is exactly `None` (default/off) and explicit `'domain_area'`; **booleans stay rejected**; `'domain_area'` uses **exact clipped domain-area weights** with zero-area rows kept. **Cross-site work standardizes externally once and passes `None`.**
- **OP-23** is reserved, its content **absent**, and **nonblocking** (**D-60**). Its descriptive phrase is already gone from this file; it survives only as the register's own quotation at A-36, which is not edited. It is **not** a WP5 opening precondition.
- **G-A** is historical/nonblocking tooling debt and **G-B is closed** by D-54 (**D-60**). No new audit apparatus is built merely to revise G-A's old count.
- **The seam-set artifact and `ρ_w` are RETIRED as opening preconditions** for WP1 and WP3–WP10 (**D-59**); per-package `scope` is declared in the manifest. S_WP2's retroactive declaration stands as history. The coverage-declaration principle is discharged **mechanically**, by instruments that print their own populations — `pin_compare`'s `compared=n/m` with `candidate_only`/`baseline_only`, the touched-file ruff population, the capture population.
...
- **Next free: A-61, D-64, CI-11, OP-33.** Machine-independent gates run in CI under **D-63**; the pin battery stays manual. Live document gates live under `tools/gates/`; the ASCII sweep runs on the conda job because it imports `bstpp.config`; Check A hashes canonical newlines, not the checkout; frozen citations of `results/_a*.py` resolve through the alias table.
...
**Two pin baselines exist and they gate different populations — say which one a gate line read.** `pin_check_v2.py` emits **six** configurations, including `hawkes_notched_4to1_polygon_mode` and `hawkes_notched_4to1_rectangle_mode` over one 4:1 **non-rectangular** domain, so **both excitation support modes are pinned** and the mode switch is the only difference between those two records; the mass table's SHA-256 is *inside* the pinned record, so a differently-built table is a DRIFT (D-27/A-11). **(1) The 2026-08-polygon baseline** `refactor-patches/baselines-2026-08-polygon/pins.json` is **FORWARD only** — dated at its capture, certifying nothing before it. **(2) The canonical 2026-07 baseline does NOT gate the polygon regime**: a six-configuration candidate reads `PIN_DIFFS 0 PARTIAL compared=4/6` against it and names both `candidate_only` keys, so **any polygon-regime gate line must pass `--baseline refactor-patches/baselines-2026-08-polygon/pins.json` explicitly** or it reports success while staying silent about the surface it was meant to cover. A `PARTIAL` still exits 0, deliberately.

**No figure computed under `standardize_cov=True` is comparable to one computed under `'domain_area'`** — not log-likelihoods, AIC, `w`, `b_0`, or the excitation share. Re-derive, do not re-label; this applies to figures already produced. All six golden pins carry no covariates, so no pin is sensitive to it. **And `'domain_area'` makes the design matrix a function of the domain**, so the same column is standardized differently per fitted site and `w` is not comparable across sites — the covariate-level analogue of why the spatial trigger must be isotropic in *real* units. **Settled at A-54 (D-54)**: `'domain_area'` is an in-package convenience for **one fitted domain**, so multi-site, district-wide and city-wide work **fixes one reference distribution externally and passes `None`**. The A-40/A-41 magnitude stands on the record and is not reinterpreted as noise.
