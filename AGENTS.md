...
Fork of [imanring/BSTPP](https://github.com/imanring/BSTPP.git) — Bayesian spatiotemporal point processes (LGCP, Hawkes, Cox-Hawkes, with optional spatial covariates) on numpyro/JAX — adapted for Philadelphia illegal-dumping analysis. Original model math is in the README; API doc in `docs/bstpp_API_doc.pdf`; the frozen Phase 3 contracts and acceptance matrix are in `docs/phase3_record.tex`. Work happens on branch `refactor` with small, atomic commits. Bug fixes follow the test-first sequence required below: commit the failing regression test before the implementation commit. Commit bodies explain the bug, the change, and the verification (read `git log` for precedent before committing).
