...
| `cox_hawkes` | `Hawkes_Model(..., cox_background=True, **PRIORS)` (was `"cox"` before CI-10/A-49; the same model, since both were truthy — the pin's values did not move) | Cox–Hawkes; same unit rectangle; latents include `z_temporal/z_seasonal/z_spatial` from seeded trace | as above + VAE decode fields `f_t/f_a/f_xy` + Cox background integral | same as hawkes, plus plain-Hawkes covariate `w` path | same string pins + z-grads | same | same |
