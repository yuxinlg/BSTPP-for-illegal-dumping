...
**Candidate tip:** Phase 3f (opened at `fe44de4`; A-22/A-23/D-35; A-24 adopts D-36–D-39 / OP-14–OP-17; A-25 records WP1 execution and corrects the WP1.4c suite figures; A-26 establishes D-40 and closes OP-17; A-27 refines D-40 to owners-by-quantity; A-28 adds `CI-6` and opens OP-21; A-29 extends the ASCII sweep and opens OP-22; A-30 renumbers the config invariants to `CI-n`; A-31 establishes D-41, the evidence-and-provenance standard; A-32 partitions the suite into fast/slow lanes and corrects the timing attribution)  
**Iteration:** 3f-WP1 + A-24 register adoption + A-25 execution record + A-26/A-27/A-28 error-identity unification (WP1.4e-1 … WP1.4f) + A-29 gate coverage + A-30 label renumbering + A-31 / D-41 evidence standard + A-32 suite lanes  
...
| D-41 (enforcement) | Every enforcement row demonstrated **red** against the pre-change state, capture committed; the revert is **minimal** (reverting shared API alongside the defect yields an `ImportError` at collection — a red that proves nothing); both-sides passes recorded as **non-discriminating by construction**, never offered as evidence; every capture records the **checked process's own exit status**, not a pipeline's | active | process; every enforcement commit for the rest of 3f | all | A-28 RED capture (`results/_a28_red_builder_max_sigma_rows.txt`, 3 failed / 1 passed with `polygon_mass.py` **alone** reverted); A-29 discrimination demo rerun after it recorded `EXIT:0` beside `FAIL` | direct | — | keep | 3f |
| D-41 (provenance) | After committing, `git status --porcelain` empty **and** `git show --stat HEAD` compared against the file list stated **before** the commit; a commit not compared to a pre-stated list is **ungated regardless of gate results**. `stderr` never routed to `/dev/null` on a git command | active | process | all | **A-28 landed with no production code while every gate was green** — one bad pathspec aborted the whole `git add`, stderr was discarded, and `" M"` was misread as staged; caught by a later restore check, amended before push | direct | **closes the dual of A-25**: A-25 covers tree-larger-than-commit (untracked files in captures); this covers commit-smaller-than-tree, which no gate in the profile could see | keep | 3f |
| D-41 (relabeling corollary) | Relabeling in place permitted where **no assertion changes** and the head note carries the **complete old→new mapping**; anything changing an assertion uses `\supsd` per occurrence. Claims of different strength are checked at their own strength (production AST-identical; test files string-blanked structural match) | active | register + rebaseline records | doc | A-30 `I1`–`I6` → `CI-1`–`CI-6`, 44 occurrences, one head note per subsection carrying the full mapping; `results/_a30_ast_equivalence.py`, `results/_a30_label_check.py` | direct | — | recorded so WP3 does not re-litigate it | 3f |
...
| D-42 / normalise-on-store | Validated numeric values are WRITTEN BACK: `object.__setattr__` for all ten `NumericalConfig` numeric fields; local rebinding in `build_quad_table` (not a dataclass, so `__setattr__` cannot reach it). Closes both instances of validate-a-copy/use-the-original | active | `config.__post_init__`; `polygon_mass.build_quad_table`, `prepare_polygon_mass_table` | ctor + set_window + builders | A-34 Lane B rows; `PIN_DIFFS 0 MATCH` | **verified** | builder rebinding is **non-discriminating given A-33** (accept set {int,float,np.float64} all coerce alike) — defensive, not corrective | keep | 3f |
| OP-22 | Clause slots rendered caller values with `!r`, so a non-ASCII argument reached a raised message | **CLOSED** at A-34 | `config._ascii_safe` on every clause return | all | A-29 sweep reports zero propagating slots | **verified** | grew 3→7 slots at A-33, crossing the A-29 trigger | closed | — |
| D-42 / CI-7 | Config-owned REAL arguments are `int` or `float`; `bool` rejected (int subclass), `np.float64` accepted (float subclass). Checked where the caller's argument still exists, not after `float()` erased it | active | `config.require_config_real`; routed at `resolve_sigma_bounds` (3 gates), `prepare_polygon_mass_table` (panel_h_m, min_sigma, max_sigma), `build_quad_table` (sigma pair, h_panel) | ctor + set_window + builders | census before/after (`refactor-patches/phase3f/op20/census_*.json`); 17 Lane B rows | **verified** (8 classes x 28 path-pairs measured) | — | keep; WP2–WP5 inherit | 3f |
| D-42 / CI-8 | Config-owned INTEGRAL arguments are `int` only; `bool` rejected. A bare `int()` accepted `16.7` (truncating to 16) and `True` (order 1) at both builders | active | `config.require_config_integral`; routed at both builders | builders | Lane B CI-8 rows | **verified** | **not in the brief** — the Step 1 census covered only `NumericalConfig.create`, so the 'integral already correct' negative result was false | keep | 3f |
| OP-20 | resolver coerced with `float()` while the config rejected with `_require_real` | **CLOSED** at A-33 | — | — | census diff | verified | A-27 stated nine input classes; measured eight named plus an unbounded `__float__` set — row corrected at A-33 | closed | — |
| OP-23 | Ownerless cutoff cluster: `window`, `spatial_window`, three tolerances, two design scales, four raise sites, no config object | open | `cutoffs.py`, `main.set_window` | ctor + set_window | — | inferred | belongs with the cutoff-provenance seam, not WP2 | **WP5**. If WP5 concludes a SIXTH config object is needed, that is a deviation from A-21's five and takes an amendment — it must not arrive as an implementation detail | 3f |
| Suite lanes (A-32) | Suite partitioned by the `slow` marker: **567 fast + 12 slow = 579**, verified an exact partition. Per-commit gate `pytest tests/ -m "not slow"` (3m43s); FULL suite at series boundaries and on any commit touching polygon mass/conservation, packaging metadata, SBC, the pair builder, or SVI/NUTS entry. **Which lane ran is stated in the commit body.** Bare `pytest tests/` still collects all 579 — no `addopts` deselects by default | active | `pyproject.toml` markers (pre-existing, previously declarative only — 6 tests marked, nothing deselecting them); 5 more marked at A-32 | all | fast lane 3m43s / 565 passed 1 skipped 12 deselected 1 xfailed EXIT:0; full 28m14s / 576 passed; collection 567+12=579 | **verified** (both lanes measured at `29058dc`) | — | keep | 3f |
| Suite timing attribution (A-32 correction) | Suite runtime is dominated by **test content, not machine health**: 10 tests = 64.8% of 1694 s; remaining 569 average 1.05 s; two polygon-conservation parametrisations alone are 32.1% | corrected | — | — | `--durations=10` at `29058dc`; phase timing (import stack 5.5 s, collection 5.1 s) rules out filter/sync tax; repo is **not** under the Box tree | **verified** | A-27/A-28/A-30 attributed ~27 min suites to memory pressure without measuring; the "~1 min" in AGENTS.md was itself stale (predates the SBC / packaging / conservation tests) | corrected at A-32; measure `--durations` before attributing a slow gate to the environment | 3f |
