...
| `a53_ci7_red.log` | **CI-7's D-41 clause-1 red at the `sp_var_mu` site**, against `db55ee1`: `11 failed, 7 passed`, `PYTEST_EXIT:1`. The rejection rows fail because `float()` accepts bools and numeric strings without complaint. **The seven passes are non-discriminating by construction** (D-41 clause 3) and are not offered as evidence — they are the accept-set rows, the default row, and the clause-identity row, and **the last one passes precisely because CI-7's clause already existed**, which is the entry's claim about the number demonstrated rather than asserted. |
| `a53_citation_sweep_red.log` | `results/_a25_citation_sweep.py` red at A-53, all three unreachable citations this commit's own not-yet-staged evidence. |
| `a52_content_checks_ordering_red.log` | **Decision-row monotonicity, red at A-52**: `section 8 decision rows are not strictly increasing: D-49 follows D-52`, `FAIL 1`, exit 1. D-52's row was drafted immediately after D-48's, next to the clauses it relates to, which reads naturally and breaks the ordering the register depends on. Fixed by moving the row after D-51. **The second commit running, a content check has caught this author rather than a historical defect** — which is the argument for running the gates on DOC commits at all. |
| `a51_content_checks_red.log` | **Check 4c firing on A-51's own text.** `results/_a25_content_checks.py` red at A-51: `4c cited-with-no-row : ['D-52']`, `FAIL 1`, exit 1. The A-51 draft cited `D-52` — a decision the *next* commit establishes — inside the very entry that withdraws A-49's citation to a non-existent branch. **The gate caught the author committing the defect the entry was written to repair**, which is the strongest form of evidence that a gate discriminates. Fixed by removing the forward reference, not by landing D-52 early. |
